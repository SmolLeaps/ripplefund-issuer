...
const unSignedVCOuput: VCBuildUnsignedOutput = {
...
describe("Issuance Component Test", () => {
  ...
  test("Input fields and validation", async () => {
    ...
    jest
      .spyOn(ApiService, "issueUnsignedVC")
      .mockReturnValue(Promise.resolve(unSignedVCOuput));

    fireEvent.change(givenNameField, { target: { value: givenName } });
    fireEvent.change(familyNameField, { target: { value: familyName } });
    fireEvent.change(dateOfIssuanceField, {
      target: { value: dateofIssuance },
    });
    fireEvent.change(drivingLicenseIDField, {
      target: { value: drivingLicenseID },
    });
    ...
    expect(window.alert).toHaveBeenCalledWith(
      `You have successfully submitted your application. (Unsigned VC successfully created.)`
    );
  });
